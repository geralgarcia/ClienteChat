        ...
        Javalin.create()
            ...
            .ws("/chat", ws -> {
                ws.onConnect(session -> {
                    String username = "Usuario " + nextUserNumber++;
                    userUsernameMap.put(session, username);
                    broadcastMessage("Servidor ", (username + " se ha unido este chat "));
                });
                ws.onClose((session, status, message) -> {
                    String username = userUsernameMap.get(session);
                    userUsernameMap.remove(session);
                    broadcastMessage("Servidor ", (username + " ha abandonado el chat"));
                });
